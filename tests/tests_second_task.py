...
class TestBookCRUD:
    ...
    def test_get_all_books(self):
        ...
        client.post(
            "/api/v1/books", json={"title": f"Book 1_{unique_id1}", "rating": 7.0}
        )
        client.post(
            "/api/v1/books", json={"title": f"Book 2_{unique_id2}", "rating": 8.0}
        )
    ...
    def test_update_book(self):
        ...
        create_resp = client.post(
            "/api/v1/books", json={"title": unique_title_old, "rating": 7.0}
        )
        book_id = create_resp.json()["id"]

        update_resp = client.put(
            f"/api/v1/books/{book_id}", json={"title": unique_title_new}
        )
    ...
    def test_delete_book(self):
        ...
        create_resp = client.post(
            "/api/v1/books", json={"title": unique_title, "rating": 7.0}
        )
        book_id = create_resp.json()["id"]
...
class TestContributorCRUD:
    ...
    def test_get_contributor_by_id(self):
        ...
        create_resp = client.post(
            "/api/v1/contributors", json={"full_name": "Jane Smith"}
        )
...
class TestBookWithRelationships:
    ...
    def test_create_book_with_genres_and_contributors(self):
        ...
        genre1 = client.post(
            "/api/v1/genres", json={"name": f"Fantasy_{unique_id}"}
        ).json()
        genre2 = client.post(
            "/api/v1/genres", json={"name": f"Adventure_{unique_id}"}
        ).json()

        author = client.post(
            "/api/v1/contributors", json={"full_name": f"Test Author_{unique_id}"}
        ).json()
        editor = client.post(
            "/api/v1/contributors", json={"full_name": f"Test Editor_{unique_id}"}
        ).json()
