...
def fill_database():
    ...
    try:
        ...
        if (
            ...
            or existing_genres_count > 0
            or existing_contributors_count > 0
        ):
        ...
        db.execute(book_genre.insert().values(book_id=books[0].id, genre_id=horror.id))
        db.execute(
            book_genre.insert().values(book_id=books[1].id, genre_id=thriller.id)
        )
        db.execute(
            book_genre.insert().values(book_id=books[2].id, genre_id=adventure.id)
        )
        db.execute(
            book_genre.insert().values(book_id=books[3].id, genre_id=historical.id)
        )

        db.execute(
            book_contributor.insert().values(
