                ...
                try:
                    for genre_data in batch:
                        try:
                            ...
                            if genre_by_id:
                                genre_by_id.name = genre_data['name']
                                logger.debug("Обновлен жанр по ID: %s", genre_data['name'])
                                success_count += 1
                            else:
                                genre_by_name = db.query(Genre).filter(Genre.name == genre_data['name']).first()
                                ...
                                if genre_by_name:
                                    genre_by_name.id = genre_data['id']
                                    logger.debug("Обновлен ID жанра для: %s", genre_data['name'])
                                    success_count += 1
                                else:
                                    genre = Genre(
                                        id=genre_data['id'],
                            ...
                            continue
                    db.commit()
                    logger.info("Обработана пачка %d-%d: %d/%d успешно", 
                    ...
                    # Запасной вариант - индивидуальная обработка
                    success_count = 0
                    for genre_data in batch:
                        if self._process_single_genre(db, genre_data):
                            success_count += 1
                    
                    logger.info("Обработана пачка %d-%d с индивидуальными коммитами: %d/%d успешно", 
                               i + 1, i + len(batch), success_count, len(batch))
